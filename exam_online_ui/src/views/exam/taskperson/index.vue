    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['exam:taskperson:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['exam:taskperson:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['exam:taskperson:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          @click="handleExport"
          v-hasPermi="['exam:taskperson:export']"
        >导出</el-button>
      </el-col>
...
<script>
import { listTaskperson, getTaskperson, delTaskperson, addTaskperson, updateTaskperson, exportTaskperson } from "@/api/exam/taskperson";
...
export default {
  ...
  created() {
    ...
    this.getDicts("${column.dictType}").then(response => {
      this.examPersonOptions = response.data;
    });
  },
  methods: {
    /** 查询考试人员列表 */
    ...
    // 考试人员字典翻译
    examPersonFormat(row, column) {
      return this.selectDictLabel(this.examPersonOptions, row.examPerson);
    },
    // 取消按钮
    cancel() {
      this.open = false;
